...
export class CompanyKnowledgeChangeService {
  public compareProjectionRuns(params: {
    accountId: string;
    fromRunId: string;
    toRunId: string;
  }): KnowledgeChangeReport {
    const fromRun = companyKnowledgeStore.requireProjectionRun(
      params.accountId,
      params.fromRunId
    );
    const toRun = companyKnowledgeStore.requireProjectionRun(
      params.accountId,
      params.toRunId
    );
    ...
    const fromClaims = fromRun.claimIds
      .map((id) => companyKnowledgeStore.getClaim(params.accountId, id))
      .filter((claim): claim is KnowledgeClaim => Boolean(claim));
    const toClaims = toRun.claimIds
      .map((id) => companyKnowledgeStore.getClaim(params.accountId, id))
      .filter((claim): claim is KnowledgeClaim => Boolean(claim));
  ...
  public changesSince(params: {
    ...
    const runs = companyKnowledgeStore
      .listProjectionRuns({
        accountId: params.accountId,
        limit: 500,
      })
    ...
    const claims = companyKnowledgeStore
      .listClaims({
        accountId: params.accountId,
        currentOnly: false,
        limit: 1000,
      })
      .filter((claim) => claim.observedAt >= params.since);

    const entities = companyKnowledgeStore
      .listEntities({
        accountId: params.accountId,
        limit: 500,
      })
      .filter((entity) => entity.lastObservedAt >= params.since);
...
export const companyKnowledgeChangeService =
  new CompanyKnowledgeChangeService();
