...
async function resolveEntityFromRow(params: {
  accountId: string;
  datasetId: string;
  version: DatasetVersion;
  table: DatasetTable;
...
export class StructuredKnowledgeRuntimeProjectionService {
  public async projectDataset(params: {
    accountId: string;
    datasetId: string;
    versionId?: string;
    ...
    await companyKnowledgePersistence.saveProjectionRun(run);
    ...
    try {
      ...
      const observedAt = version.createdAt;
        ...
        for (const table of version.tables) {
          ...
          for (const [rowIndex, row] of table.rows.entries()) {
            ...
            for (const role of roles) {
              const entity = await resolveEntityFromRow({
                accountId: params.accountId,
            ...
            const addRelationship = async (
              ...
              const relationship =
                await companyKnowledgePersistence.upsertRelationship({
                  accountId: params.accountId,
            ...
            await addRelationship('CUSTOMER', 'PLACED', 'ORDER');
            await addRelationship('ORDER', 'CONTAINS', 'PRODUCT');
            await addRelationship('PRODUCT', 'SUPPLIED_BY', 'SUPPLIER');
            await addRelationship('ORDER', 'BELONGS_TO', 'BRANCH');
            ...
            if (primaryRecord) {
              ...
              for (const fact of resolvedFactColumns) {
                ...
                const claim = await companyKnowledgePersistence.recordClaim({
                  accountId: params.accountId,
              ...
              const event = await companyKnowledgePersistence.recordEvent({
                accountId: params.accountId,
                type:
        ...
        const projectedEvent = await companyKnowledgePersistence.recordEvent({
          accountId: params.accountId,
      ...
      await companyKnowledgePersistence.saveProjectionRun(completed);
      return completed;
    } catch (error: any) {
      ...
      await companyKnowledgePersistence.saveProjectionRun(failed);
      throw error;
    }
  }
}

export const structuredKnowledgeRuntimeProjectionService =
  new StructuredKnowledgeRuntimeProjectionService();
