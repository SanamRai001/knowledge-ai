...
async function upsertDocumentEntity(params: {
  accountId: string;
  kb: KnowledgeBase;
  document: KnowledgeDocument;
  pageNumber: number;
...
export class DocumentKnowledgeRuntimeProjectionService {
  public async projectKnowledgeBase(params: {
    accountId: string;
        ...
        for (const document of kb.documents) {
          ...
          for (const page of document.pages) {
            ...
            for (const line of lines) {
              ...
              if (labeledEntity) {
                const entity = await upsertDocumentEntity({
                  accountId: params.accountId,
                  kb,
                  document,
                  pageNumber: page.pageNumber,
              ...
              for (const match of relationshipMatches(line)) {
                const subject = await upsertDocumentEntity({
                  accountId: params.accountId,
                  kb,
                  document,
                  pageNumber: page.pageNumber,
                ...
                const object = await upsertDocumentEntity({
                  accountId: params.accountId,
                  kb,
                  document,
                  pageNumber: page.pageNumber,
              ...
              if (claim) {
                ...
                const storedClaim = await companyKnowledgePersistence.recordClaim({
                  accountId: params.accountId,
                  subjectEntityId: subject.id,
...
export const documentKnowledgeRuntimeProjectionService =
  new DocumentKnowledgeRuntimeProjectionService();
