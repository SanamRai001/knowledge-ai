...
export class WatchDraftRuntimeService {
  ...
  public async selectTarget(params: {
    ...
    const draft = await watchPersistence.requireDraft(
      params.accountId,
      params.draftId
    );
    ...
    const validated = watchRuntimeService.validateCondition(
      params.accountId,
      condition
    );

    return watchPersistence.updateDraft(
      params.accountId,
      draft.id,
      {
        status: 'PROPOSED',
  ...
  public async save(params: {
    accountId: string;
    draftId: string;
  }): Promise<{ draft: WatchDraft; ruleId: string }> {
    const draft = await watchPersistence.requireDraft(
      params.accountId,
      params.draftId
    );
    ...
    if (draft.expiresAt <= Date.now()) {
      await watchPersistence.updateDraft(params.accountId, draft.id, {
        status: 'INVALID',
      });
      throw new WatchDraftError(
        'WATCH_DRAFT_EXPIRED',
    ...
    const condition = watchRuntimeService.validateCondition(
      params.accountId,
      draft.condition
    );
    ...
    const savedDraft = await watchPersistence.updateDraft(
      params.accountId,
      draft.id,
      {
        status: 'SAVED',
  ...
  private async resolveParsed(params: {
    accountId: string;
    instruction: string;
    ...
    if (request.kind === 'TIME_REACHED') {
      ...
      return watchPersistence.createDraft({
        accountId: params.accountId,
...
export const watchDraftRuntimeService =
  new WatchDraftRuntimeService();
