...
function broaderEntityCandidates(
  accountId: string,
  type: CompanyEntityType,
  reference: string
): CompanyEntity[] {
  return companyKnowledgeStore.listEntities({
    accountId,
    type,
    search: reference,
    limit: 20,
...
export class ActionProposalService {
  public createFromParsed(params: {
    accountId: string;
    instruction: string;
  ...
  private resolveSingleEntity(params: {
    accountId: string;
    type: CompanyEntityType;
    reference: string;
  }):
  ...
  private resolvePaymentOrder(params: {
    accountId: string;
    parsed: ParsedActionInput;
  }):
    ...
    if (params.parsed.orderReference) {
      const order = this.resolveSingleEntity({
        accountId: params.accountId,
        type: 'ORDER',
    ...
    const customer = this.resolveSingleEntity({
      accountId: params.accountId,
      type: 'CUSTOMER',
    ...
    const placed = companyKnowledgeStore
      .listRelationships({
        accountId: params.accountId,
    ...
    for (const relationship of placed) {
      const order = companyKnowledgeStore.getEntity(
        params.accountId,
        relationship.objectEntityId
      );
      if (!order || order.type !== 'ORDER') continue;

      const balance = effectiveCompanyStateService.resolve(
        params.accountId,
        order.id,
        'BALANCE_DUE'
      );
    ...
    const candidates =
      openOrders.length > 0
        ...
        : placed
            .map((relationship) =>
              companyKnowledgeStore.getEntity(
                params.accountId,
                relationship.objectEntityId
              )
            )
            ...
            .map((order) =>
              entityCandidate(order, 'Order placed by this customer.')
            );
  ...
  private proposePayment(
    ...
    const target = this.resolvePaymentOrder({
      accountId: params.accountId,
    ...
    const paid = effectiveCompanyStateService.resolve(
      params.accountId,
      target.order.id,
      'PAID_AMOUNT'
    );
    const total = effectiveCompanyStateService.resolve(
      params.accountId,
      target.order.id,
      'TOTAL_AMOUNT'
    );
  ...
  private proposeInventoryReceipt(
    params: {
      accountId: string;
      instruction: string;
      parsed: ParsedActionInput;
    ...
    const product = this.resolveSingleEntity({
      accountId: params.accountId,
      type: 'PRODUCT',
    ...
    const stock = effectiveCompanyStateService.resolve(
      params.accountId,
      product.entity.id,
      'CURRENT_STOCK'
    );
