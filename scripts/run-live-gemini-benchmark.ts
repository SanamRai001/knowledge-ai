...
const GEMINI_MIN_INTERVAL_MS = Number(process.env.GEMINI_BENCHMARK_INTERVAL_MS || 13_000);
const GEMINI_MAX_ATTEMPTS = Number(process.env.GEMINI_BENCHMARK_MAX_ATTEMPTS || 3);
const GEMINI_RETRY_BACKOFF_MS = Number(process.env.GEMINI_BENCHMARK_RETRY_BACKOFF_MS || 20_000);
let lastGeminiAttemptAt = 0;

const shouldExerciseGemini = (testCase: BenchmarkCase) =>
  !testCase.shouldRefuse && testCase.category !== 'TABLE' && testCase.category !== 'ADVERSARIAL';

async function paceGeminiAttempt() {
  const waitMs = Math.max(0, GEMINI_MIN_INTERVAL_MS - (Date.now() - lastGeminiAttemptAt));
...
async function run() {
  ...
  for (const testCase of cases) {
    ...
    const expectsGemini = shouldExerciseGemini(testCase);
    let result: Awaited<ReturnType<typeof knowledgeCognitiveEngine.answerQuestion>> | null = null;
    let attempts = 0;

    do {
      if (expectsGemini) await paceGeminiAttempt();
      attempts += 1;
      result = await knowledgeCognitiveEngine.answerQuestion({
      ...
      if (expectsGemini && result.engineUsed !== 'gemini-3.8-flash' && attempts < GEMINI_MAX_ATTEMPTS) {
        console.warn(`Gemini fallback for ${testCase.id} on attempt ${attempts}; retrying after backoff.`);
        await sleep(GEMINI_RETRY_BACKOFF_MS);
      }
    } while (expectsGemini && result.engineUsed !== 'gemini-3.8-flash' && attempts < GEMINI_MAX_ATTEMPTS);
  ...
  const geminiEligible = results.filter((item) => item.expectsGemini);
  const geminiResponses = geminiEligible.filter((item) => item.engineUsed === 'gemini-3.8-flash').length;
  const geminiCoverage = geminiEligible.length ? geminiResponses / geminiEligible.length : 0;
  ...
  const summary = {
    ...
    geminiEligibleCases: geminiEligible.length,
    geminiResponses,
    geminiCoverage,
    pacing: {
      minIntervalMs: GEMINI_MIN_INTERVAL_MS,
      maxAttempts: GEMINI_MAX_ATTEMPTS,
      retryBackoffMs: GEMINI_RETRY_BACKOFF_MS,
    },
    engineDistribution,
  ...
  if (geminiResponses === 0) {
    throw new Error('Live benchmark invalid: zero Gemini-eligible cases were generated by Gemini. The engine fell back to deterministic mode for every eligible case.');
  }
  if (geminiCoverage < 0.8) {
    throw new Error(`Live benchmark incomplete: Gemini covered only ${(geminiCoverage * 100).toFixed(1)}% of eligible cases after pacing/retries.`);
  }
}
