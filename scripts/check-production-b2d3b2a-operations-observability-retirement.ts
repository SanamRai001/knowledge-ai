...
async function main() {
  ...
  try {
    ...
    assert(
      ...
      'B2D3B2A must preserve explicit non-production compatibility for the later tenant/SaaS DEVELOPMENT_ONLY group.'
    );
  } finally {
  ...
  console.log(
    'Operations and observability HTTP registrations are removed, both route families are permanently retired, operational/telemetry services remain directly callable, system/provider read-only compatibility remains registered, and later tenant/SaaS compatibility remains untouched.'
  );
}
